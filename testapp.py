...
# Extract credentials from Hugging Face secrets
...
# FIX: Switched 'authkey' parameter to native, inline 'user' and 'pw' 
# combined with the 'HTTP' access method configuration to bypass broken Java JAR classpaths.
config_content = f"""
...
def get_sas_session():
    try:
        # Establish connection using the web driver profile
        sas = saspy.SASsession(cfgfile=config_file_path, cfgname="oda")
        return sas
    except Exception as e:
        # Displays the exact component error if it fails
